...
<%  ArrayList<sellbuyDTO> sellbuyss =(new sellbuyDAO()).getfuall();
...
<div class="container my-3">
    <table class="table">
		...
		<%
		 ArrayList<sellbuyDTO> pagess =(new sellbuyDAO()).getallfurn(startRow,pageSize);
...
<%if(cnt != 0){
   ...
   %>
   <a href="boardfurnall.jsp?pageNum=<%=1%>">처음으로</a>
   <%
   if(startPage>pageBlock){
   %>
   <a href="boardfurnall.jsp?pageNum=<%=startPage-pageBlock%>">prev</a>
   <% } %>
   <%for(int i=startPage;i<=endPage;i++){ %>
   <a href="boardfurnall.jsp?pageNum=<%=i%>"><%=i %></a>
   <%} %>
   <%if(endPage<pageCount){ %>
      <a href="boardfurnall.jsp?pageNum=<%=startPage+pageBlock%>">Next</a>
      <%} %>
      <a href="boardfurnall.jsp?pageNum=<%=pageCount%>">마지막으로</a>
      <%} %>
</div>
