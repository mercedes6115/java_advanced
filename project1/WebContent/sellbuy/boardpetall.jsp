...
<%  ArrayList<sellbuyDTO> sellbuyss =(new sellbuyDAO()).getptall();
...
<div class="container my-3">
    <table class="table">
		...
		<%
		 ArrayList<sellbuyDTO> pagess =(new sellbuyDAO()).getallpet(startRow,pageSize);
...
<%if(cnt != 0){
   ...
   %>
   <a href="boardpetall.jsp?pageNum=<%=1%>">처음으로</a>
   <%
   if(startPage>pageBlock){
   %>
   <a href="boardpetall.jsp?pageNum=<%=startPage-pageBlock%>">prev</a>
   <% } %>
   <%for(int i=startPage;i<=endPage;i++){ %>
   <a href="boardpetall.jsp?pageNum=<%=i%>"><%=i %></a>
   <%} %>
   <%if(endPage<pageCount){ %>
      <a href="boardpetall.jsp?pageNum=<%=startPage+pageBlock%>">Next</a>
      <%} %>
      <a href="boardpetall.jsp?pageNum=<%=pageCount%>">마지막으로</a>
      <%} %>
</div>
