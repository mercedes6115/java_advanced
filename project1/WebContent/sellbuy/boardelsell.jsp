...
<%  ArrayList<sellbuyDTO> sellbuyss =(new sellbuyDAO()).getelsl();
...
<div class="container my-3">
    <%@ include file="/sellbuy/headerin.jsp"%>	
     <%@ include file="/sellbuy/elcate.jsp"%>	
</div>
...
<div class="container my-3">
    <table class="table">
		...
		<%
		 ArrayList<sellbuyDTO> pagess =(new sellbuyDAO()).getsellel(startRow,pageSize);
...
<%if(cnt != 0){
   ...
   %>
   <a href="boardelsell.jsp?pageNum=<%=1%>">처음으로</a>
   <%
   if(startPage>pageBlock){
   %>
   <a href="boardelsell.jsp?pageNum=<%=startPage-pageBlock%>">prev</a>
   <% } %>
   <%for(int i=startPage;i<=endPage;i++){ %>
   <a href="boardelsell.jsp?pageNum=<%=i%>"><%=i %></a>
   <%} %>
   <%if(endPage<pageCount){ %>
      <a href="boardelsell.jsp?pageNum=<%=startPage+pageBlock%>">Next</a>
      <%} %>
      <a href="boardelsell.jsp?pageNum=<%=pageCount%>">마지막으로</a>
      <%} %>
</div>
