...
<%  ArrayList<sellbuyDTO> sellbuyss =(new sellbuyDAO()).gettksl();
...
<div class="container my-3">
    <%@ include file="/sellbuy/headerin.jsp"%>	
     <%@ include file="/sellbuy/tkcate.jsp"%>	
</div>
...
<div class="container my-3">
    <table class="table">
		...
		<%
		 ArrayList<sellbuyDTO> pagess =(new sellbuyDAO()).getselltk(startRow,pageSize);
...
<%if(cnt != 0){
   ...
   %>
   <a href="boardtksell.jsp?pageNum=<%=1%>">처음으로</a>
   <%
   if(startPage>pageBlock){
   %>
   <a href="boardtksell.jsp?pageNum=<%=startPage-pageBlock%>">prev</a>
   <% } %>
   <%for(int i=startPage;i<=endPage;i++){ %>
   <a href="boardtksell.jsp?pageNum=<%=i%>"><%=i %></a>
   <%} %>
   <%if(endPage<pageCount){ %>
      <a href="boardtksell.jsp?pageNum=<%=startPage+pageBlock%>">Next</a>
      <%} %>
      <a href="boardtksell.jsp?pageNum=<%=pageCount%>">마지막으로</a>
      <%} %>
</div>
