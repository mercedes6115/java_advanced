...
<%  ArrayList<sellbuyDTO> sellbuyss =(new sellbuyDAO()).getitall();
...
<div class="container my-3">
    <table class="table">
		...
		<%
		 ArrayList<sellbuyDTO> pagess =(new sellbuyDAO()).getallit(startRow,pageSize);
...
<%if(cnt != 0){
   ...
   %>
   <a href="boarditall.jsp?pageNum=<%=1%>">처음으로</a>
   <%
   if(startPage>pageBlock){
   %>
   <a href="boarditall.jsp?pageNum=<%=startPage-pageBlock%>">prev</a>
   <% } %>
   <%for(int i=startPage;i<=endPage;i++){ %>
   <a href="boarditall.jsp?pageNum=<%=i%>"><%=i %></a>
   <%} %>
   <%if(endPage<pageCount){ %>
      <a href="boarditall.jsp?pageNum=<%=startPage+pageBlock%>">Next</a>
      <%} %>
      <a href="boarditall.jsp?pageNum=<%=pageCount%>">마지막으로</a>
      <%} %>
</div>
