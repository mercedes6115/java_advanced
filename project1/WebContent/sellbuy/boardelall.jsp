...
<%  ArrayList<sellbuyDTO> sellbuyss =(new sellbuyDAO()).getelall();
...
<div class="container my-3">
    <table class="table">
		...
		<%
		 ArrayList<sellbuyDTO> pagess =(new sellbuyDAO()).getallel(startRow,pageSize);
...
<%if(cnt != 0){
   ...
   %>
   <a href="boardelall.jsp?pageNum=<%=1%>">처음으로</a>
   <%
   if(startPage>pageBlock){
   %>
   <a href="boardelall.jsp?pageNum=<%=startPage-pageBlock%>">prev</a>
   <% } %>
   <%for(int i=startPage;i<=endPage;i++){ %>
   <a href="boardelall.jsp?pageNum=<%=i%>"><%=i %></a>
   <%} %>
   <%if(endPage<pageCount){ %>
      <a href="boardelall.jsp?pageNum=<%=startPage+pageBlock%>">Next</a>
      <%} %>
      <a href="boardelall.jsp?pageNum=<%=pageCount%>">마지막으로</a>
      <%} %>
</div>
