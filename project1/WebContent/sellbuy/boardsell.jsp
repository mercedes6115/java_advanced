...
<%  ArrayList<sellbuyDTO> sellbuyss =(new sellbuyDAO()).getSL();
...
<div class="container my-3">
    <table class="table">
		...
		<%
		 ArrayList<sellbuyDTO> pagess =(new sellbuyDAO()).getsell(startRow,pageSize);
...
<%if(cnt != 0){
   ...
   %>
   <a href="boardsell.jsp?pageNum=<%=1%>">처음으로</a>
   <%
   if(startPage>pageBlock){
   %>
   <a href="boardsell.jsp?pageNum=<%=startPage-pageBlock%>">prev</a>
   <% } %>
   <%for(int i=startPage;i<=endPage;i++){ %>
   <a href="boardsell.jsp?pageNum=<%=i%>"><%=i %></a>
   <%} %>
   <%if(endPage<pageCount){ %>
      <a href="boardsell.jsp?pageNum=<%=startPage+pageBlock%>">Next</a>
      <%} %>
      <a href="boardsell.jsp?pageNum=<%=pageCount%>">마지막으로</a>
      <%} %>
</div>
