...
<%  ArrayList<sellbuyDTO> sellbuyss =(new sellbuyDAO()).getptsl();
...
<div class="container my-3">
    <%@ include file="/sellbuy/headerin.jsp"%>	
     <%@ include file="/sellbuy/petcate.jsp"%>	
</div>
...
<div class="container my-3">
    <table class="table">
		...
		<%
		 ArrayList<sellbuyDTO> pagess =(new sellbuyDAO()).getsellpet(startRow,pageSize);
...
<%if(cnt != 0){
   ...
   %>
   <a href="boardpetsell.jsp?pageNum=<%=1%>">처음으로</a>
   <%
   if(startPage>pageBlock){
   %>
   <a href="boardpetsell.jsp?pageNum=<%=startPage-pageBlock%>">prev</a>
   <% } %>
   <%for(int i=startPage;i<=endPage;i++){ %>
   <a href="boardpetsell.jsp?pageNum=<%=i%>"><%=i %></a>
   <%} %>
   <%if(endPage<pageCount){ %>
      <a href="boardpetsell.jsp?pageNum=<%=startPage+pageBlock%>">Next</a>
      <%} %>
      <a href="boardpetsell.jsp?pageNum=<%=pageCount%>">마지막으로</a>
      <%} %>
</div>
