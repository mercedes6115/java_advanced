...
<%  ArrayList<sellbuyDTO> sellbuyss =(new sellbuyDAO()).getbksl();
...
<div class="container my-3">
    <%@ include file="/sellbuy/headerin.jsp"%>	
     <%@ include file="/sellbuy/bkcate.jsp"%>	
</div>
...
<div class="container my-3">
    <table class="table">
		...
		<%
		 ArrayList<sellbuyDTO> pagess =(new sellbuyDAO()).getsellbk(startRow,pageSize);
...
<%if(cnt != 0){
   ...
   %>
   <a href="boardbksell.jsp?pageNum=<%=1%>">처음으로</a>
   <%
   if(startPage>pageBlock){
   %>
   <a href="boardbksell.jsp?pageNum=<%=startPage-pageBlock%>">prev</a>
   <% } %>
   <%for(int i=startPage;i<=endPage;i++){ %>
   <a href="boardbksell.jsp?pageNum=<%=i%>"><%=i %></a>
   <%} %>
   <%if(endPage<pageCount){ %>
      <a href="boardbksell.jsp?pageNum=<%=startPage+pageBlock%>">Next</a>
      <%} %>
      <a href="boardbksell.jsp?pageNum=<%=pageCount%>">마지막으로</a>
      <%} %>
</div>
