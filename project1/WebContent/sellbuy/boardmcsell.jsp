...
<%  ArrayList<sellbuyDTO> sellbuyss =(new sellbuyDAO()).getmcsl();
...
<div class="container my-3">
    <%@ include file="/sellbuy/headerin.jsp"%>	
     <%@ include file="/sellbuy/mccate.jsp"%>	
</div>
...
<div class="container my-3">
    <table class="table">
		...
		<%
		 ArrayList<sellbuyDTO> pagess =(new sellbuyDAO()).getsellmc(startRow,pageSize);
...
<%if(cnt != 0){
   ...
   %>
   <a href="boardmcsell.jsp?pageNum=<%=1%>">처음으로</a>
   <%
   if(startPage>pageBlock){
   %>
   <a href="boardmcsell.jsp?pageNum=<%=startPage-pageBlock%>">prev</a>
   <% } %>
   <%for(int i=startPage;i<=endPage;i++){ %>
   <a href="boardmcsell.jsp?pageNum=<%=i%>"><%=i %></a>
   <%} %>
   <%if(endPage<pageCount){ %>
      <a href="boardmcsell.jsp?pageNum=<%=startPage+pageBlock%>">Next</a>
      <%} %>
      <a href="boardmcsell.jsp?pageNum=<%=pageCount%>">마지막으로</a>
      <%} %>
</div>
