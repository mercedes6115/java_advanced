...
<%  ArrayList<sellbuyDTO> sellbuyss =(new sellbuyDAO()).getclall();
...
<div class="container my-3">
    <table class="table">
		...
		<%
		 ArrayList<sellbuyDTO> pagess =(new sellbuyDAO()).getallcloth(startRow,pageSize);
...
<%if(cnt != 0){
   ...
   %>
   <a href="boardclothall.jsp?pageNum=<%=1%>">처음으로</a>
   <%
   if(startPage>pageBlock){
   %>
   <a href="boardclothall.jsp?pageNum=<%=startPage-pageBlock%>">prev</a>
   <% } %>
   <%for(int i=startPage;i<=endPage;i++){ %>
   <a href="boardclothall.jsp?pageNum=<%=i%>"><%=i %></a>
   <%} %>
   <%if(endPage<pageCount){ %>
      <a href="boardclothall.jsp?pageNum=<%=startPage+pageBlock%>">Next</a>
      <%} %>
      <a href="boardclothall.jsp?pageNum=<%=pageCount%>">마지막으로</a>
      <%} %>
</div>
