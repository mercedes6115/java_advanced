...
<table class="table">
  ...
  <tbody>
  	<div class="row" align=center>
		<% 
		ArrayList<JobDTO> jobs =(new JobDAO()).getpageList(startRow,pageSize);
			for(JobDTO job : jobs){
    ...
    <tr>
      ...
      <td><%= job.getJcon()%></td>
 <td><a href="jobboarddetail.jsp?jtitle=<%=job.getJtitle()%>&jfile=<%=job.getJfile()%>"><%=job.getJtitle()%></td>
       <td><%= job.getJuser()%></td>
       <td><%= job.getJdate()%></td>
      <td></td>
    </tr>
...
<%if(cnt != 0){
  ...
  <a href="resumeget.jsp">이력서 작성하러가기</a>
</div>
